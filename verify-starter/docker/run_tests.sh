...
function usage() {
  echo "usage: devbox.entrypoint.sh test (--ci|--local) [--all] [--lint] [--unit-test] [--no-pyenv] [--run|--skip {step} <extra_args>]"
  echo ""
...
function run_steps() {
  ...
  if [[ ${STEPS_ACTIVE_MAP[black]} == "true" ]]; then
    echo "Running black..."
    eval "black ${BLACK_ACTION} ${PROJECT_DIR} ${STEP_EXTRA_ARGUMENTS[black]} tests"
  fi
  ...
  if [[ ${STEPS_ACTIVE_MAP[pytest]} == "true" ]]; then
    echo "Running pytest..."
    eval "python_starter_CONFIG=\"testing\" pytest ${STEP_EXTRA_ARGUMENTS[pytest]}"
  fi
}
validate_input
handle_input "$@"
